...
# Проверка что запущен лишь один скрипт
if [ -e "$lock_file" ]; then
...
if [ -d "$directory" ]; then # Является ли путь директорией
    ...
    rocket=20
    while true; do
        filenames=($(ls -t "$directory" | head -n 30 2>/dev/null)) # Получение списка файлов с координатами целей 30 последних.
        for name in "${filenames[@]}"; do
            id=$(ls -t "$directory/$name" | tail -c 7 2>/dev/null) # В папке targets достаем из названия файла  id цели
